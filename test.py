...
# 🎨 Streamlit UI
# -----------------------------
st.title(" 📖 감정 도서관:오늘의 기분을 알려주세요! ✒️")
st.write("어서오세요, 감정 도서관입니다. 오늘의 기분과 고민을 알려주세요. 당신에게 위로가 될, 하나의 문학 구절을 대출해드립니다.")

# 사용자 입력
mood = st.selectbox("오늘 기분은 어때요?", ["행복함", "우울함", "지침", "답답함", "설렘"])
worry = st.selectbox("어떤 고민 때문에 오셨나요?", ["인간관계", "학업", "미래", "번아웃", "스트레스"])

# 결과 출력
if st.button("📖 오늘의 문학 구절은?"):
    key = (mood, worry)
    if key in prescriptions:
        quote, explanation = random.choice(prescriptions[key])
        st.subheader("📖 오늘의 당신에게 딱 맞는 문학 구절!")
        st.success(quote)
